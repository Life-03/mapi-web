...
import qoriEs from "../../i18n/qoriwayrachina.es.json";
import qoriEn from "../../i18n/qoriwayrachina.en.json";
export default function Page() {
  const locale = useClientLocale();
  const t = locale === "en" ? qoriEn : qoriEs;
  ...
  const [destination, setDestination] = useState('Qoriwayrachina');
  //FQS
  ...
  const fetchFinancial = async () => {
    try {
      const response = await fetch('/api?idRuta=2&idLugar=2');
  ...
  const selectedFaqs = locale === "en" ? "QoriwayrachinaEn" : "Qoriwayrachina";
  ...
  return (
    <>
      ...
      <div className="relative w-full h-screen bg-cover bg-center mt-[-80]" style={{ backgroundImage: "url('/images/qoriwayrachina.webp')" }}>
      ...
      <section className='max-w-6xl mx-auto space-y-16 px-6'>
        ...
        <div className="pt-16 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <h3>{t["section.intro.subtitle"]}</h3>
            <p>
              {t["section.intro.p1"]}
              <br/>
              {t["section.intro.p2"]}
              <br/><strong>{t["section.intro.strong"]}</strong>
            </p>
          </div>

          <div>
            <h3>{t["section.access.title"]}</h3>
            <span>
              <strong className='text-gray-600'>{t["section.access.lead"]}</strong>
              <ul className="list-disc pl-8 text-gray-600 my-4">
                {t["section.access.items"].map((item) => (
        ...
        <div>
          <h2 className='lg:text-center my-4 '>{t["section.availability.title"]}</h2>
          <p className='text-center'>
            {t["section.availability.p1"]}
          </p>
        ...
        <div>
          <h2 className="text-center">{t["section.info.title"]}</h2>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            {/* Recuadro de alerta */}
            <div className="lg:w-1/3 bg-yellow-100 border-l-4 border-secondary text-blue-700 px-4 py-3 shadow-lg" role="alert">
              <p className="font-bold text-primary">{t["section.info.alert.title"]}</p>
              <p className="text-sm mt-1">
                {t["section.info.alert.p1"]}
              </p>
            </div>
          </div>
        </div>
        ...
        <div>
          <h2 className="text-center">{t["section.faq.title"]}</h2>
