...
import chachaEs from "../../i18n/chachabamba.es.json";
import chachaEn from "../../i18n/chachabamba.en.json";

export default function Page() {
  const locale = useClientLocale();
  const t = locale === "en" ? chachaEn : chachaEs;
  ...
  const [destination, setDestination] = useState('Chachabamba');
  //PREGUNTAS FRECUENTES
  const selectedFaqs = locale === "en" ? "ChachabambaEn" : "Chachabamba";
  ...
  const fetchFinancial = async () => {
    try {
      const response = await fetch('/api?idRuta=5&idLugar=2');
  ...
  return (
    <>
      <div className="relative w-full h-screen bg-cover bg-center mt-[-80]" style={{ backgroundImage: "url('/images/chachabamba.webp')" }}>
      ...
      <div className='max-w-6xl mx-auto space-y-16 px-6'>
        <TourList category="inca-trail" slug="short-inca-trail" />
        ...
        <div className="pt-16 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <ul className="list-disc pl-8 text-gray-600 my-4">
              {t["section.location.list"].map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </div>
